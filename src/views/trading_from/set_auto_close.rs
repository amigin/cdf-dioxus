...
pub fn render_set_auto_close(cx: Scope) -> Element {
    ...
    //let (tp_mode, sl_mode) = trading_panel_state.read().get_tp_sl_mode(&instrument_id);
    ...
    let (tp_value, sl_value) = if let Some((tp, sl)) = &tp_sl {
        let tp_value = if let Some(tp_value) = &tp {
            ...
            use_state(cx, || TpSlValue::Currency("".to_string()))
        };
        ...
        let tp_value = use_state(cx, || TpSlValue::Currency("".to_string()));
        let sl_value = use_state(cx, || TpSlValue::Currency("".to_string()));
                ...
                div { style: "margin-top: 15px",
                    ...
                    div { class: "btn-group",
                        span {
                            ...
                            onclick: move |_| {
                                tp_value.set(tp_value.get().toggle());
                            },
                            tp_value.get_sign()
                        }
                        input {
                            class: "form-control form-control-sm form-control-sm-left-attach",
                ...
                div { style: "margin-top: 15px",
                    ...
                    div { class: "btn-group",
                        span {
                            ...
                            style: "cursor: pointer",
                            onclick: move |_| {
                                sl_value.set(sl_value.get().toggle());
                            },
                            sl_value.get_sign()
                        }
                        input {
                    ...
                    button {
                        ...
                        onclick: move |_| {
                            ...
                            if tp.len() == 0 {
                                ...
                                write.set_tp(&instrument_id, Some(tp_value.get().clone()));
                            }
                            if sl.len() == 0 {
                                write.set_sl(&instrument_id, None);
                            } else {
                                write.set_sl(&instrument_id, Some(sl_value.get().clone()));
                            }
